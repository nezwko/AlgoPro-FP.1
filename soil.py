...
class WaterTile(pygame.sprite.Sprite):
    def __init__(self,pos,surf,groups):
        ...
        self.image = surf
        self.rect = self.image.get_rect(topleft = pos)
        self.z = LAYERS['Watered Soil']

class Plant(pygame.sprite.Sprite):
    def __init__(self, plant_type, groups, soil, check_watered):
        super().__init__(groups)
        self.plant_type = plant_type
        self.frames = import_folder('graphics/fruit/corn')
        self.soil = soil
...
class SoilLayer:
    ...
    def create_hit_rects(self):
        ...
        for index_row, row in enumerate(self.grid):
            for index_col, cell in enumerate(row):
                if 'F' in cell:  # Check if the cell contains 'F'
    ...
    def get_hit(self, point):
        for rect in self.hit_rects:
            if rect.collidepoint(point):
                ...
                if 0 <= y < len(self.grid) and 0 <= x < len(self.grid[0]):  # Ensure indices are within bounds
                    if 'F' in self.grid[y][x]:  # Check if the cell contains 'F'
                        self.grid[y][x].append('X')  # Append 'X' to the cell
        ...
        for soil_sprite in self.soil_sprites.sprites():
            if soil_sprite.rect.collidepoint(target_pos):
                ...
                if 'P' not in self.grid[y][x]:
                    ...
                    # Pass the check_watered method as an argument
    ...
    def create_soil_tiles(self):
        ...
        for index_row, row in enumerate(self.grid):
            for index_col, cell in enumerate(row):
                if 'X' in cell:  # Check if the cell contains 'X'
